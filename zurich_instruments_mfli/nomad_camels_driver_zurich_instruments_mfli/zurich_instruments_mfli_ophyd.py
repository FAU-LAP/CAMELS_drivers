...
def construct_correct_path(dev_name, path):
    ...
    skip = int(path[0] == "/")
    if (
        len(path) > len(dev_name) + skip
        and path[skip : len(dev_name) + skip].lower() == dev_name
    ):
...
class Zurich_Instruments_MFLI(Device):
    ...
    connect_to_dev = Cpt(Custom_Function_Signal, name="connect_to_dev", kind="config")
    ...
    imp_50_input = Cpt(Custom_Function_Signal, name="imp_50_input", kind="config")
    ...
    # === Output Amplitude ===
    enable_output_amp = Cpt(
        Custom_Function_Signal, name="enable_output_amp", kind="config"
    )
    # === Output ===
    select_output_range = Cpt(
        Custom_Function_Signal, name="select_output_range", kind="config"
    )
    autorange_output = Cpt(
        Custom_Function_Signal, name="autorange_output", kind="config"
    )
    add_output = Cpt(Custom_Function_Signal, name="add_output", kind="config")
    diff_output = Cpt(Custom_Function_Signal, name="diff_output", kind="config")
    enable_output = Cpt(Custom_Function_Signal, name="enable_output", kind="config")
    imp_50_output = Cpt(Custom_Function_Signal, name="imp_50_output", kind="config")
    # === Filter ===
    filter_order = Cpt(Custom_Function_Signal, name="filter_order", kind="config")
    filter_time_constant = Cpt(
        Custom_Function_Signal, name="filter_time_constant", kind="config"
    )
    sinc = Cpt(Custom_Function_Signal, name="sinc", kind="config")
    # === Reference ===
    select_reference = Cpt(
        Custom_Function_Signal, name="select_reference", kind="config"
    )
    select_nth_harmonic = Cpt(
        Custom_Function_Signal, name="select_nth_harmonic", kind="config"
    )
    set_demodulator_phaseshift = Cpt(
        Custom_Function_Signal, name="set_demodulator_phaseshift", kind="config"
    )
    select_1_over_nth_harmonic = Cpt(
        Custom_Function_Signal, name="select_1_over_nth_harmonic", kind="config"
    )
    # --------------------------------------------------------------------------
    # Setting Variables
    # --------------------------------------------------------------------------
    ...
    # === Reference Frequency ===
    set_reference_frequency = Cpt(
        Custom_Function_Signal, name="set_reference_frequency"
    )
    # === Output Amplitude ===
    ...
    # --------------------------------------------------------------------------
    # === Lock-In Signal ===
    get_x = Cpt(Custom_Function_SignalRO, name="get_x", metadata={"units": "V"})
    get_y = Cpt(Custom_Function_SignalRO, name="get_y", metadata={"units": "V"})
    get_R = Cpt(Custom_Function_SignalRO, name="get_R", metadata={"units": "V"})
    get_theta = Cpt(Custom_Function_SignalRO, name="get_theta", metadata={"units": "V"})
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.filter_time_constant.put_function = self.filter_time_constant_function
        self.sinc.put_function = self.sinc_function
        # === Reference ===
        self.select_reference.put_function = self.select_reference_function
        self.select_nth_harmonic.put_function = self.select_nth_harmonic_function
        ...
        # === Reference Frequency
        self.set_reference_frequency.put_function = (
            self.set_reference_frequency_function
        )
        # === Output Amplitude ===
    ...
    def connect_to_dev_function(self, dev_name):
        ...
        try:
            ...
            try:
                self.LockIn = ziDAQServer(
                    self.props["serveraddress"],
                    self.props["serverport"],
                    self.props["apilevel"],
                )
                self.LockIn.connectDevice(self.dev_name, self.props["interfaces"][0])
            except Exception as e:
                print(
                    f"cannot connect Zurich Instruments MFLI "
            ...
            print(
                f"cannot connect Zurich Instruments MFLI ({dev_name}): "
        ...
        if not self.LockIn is None:
            if ":" in cmd and len(cmd.split(":")) == 2:
                ...
                try:
                    ...
                    print(
                        f"Zurich Instruments MFLI could not set double "
            ...
            print(
                f"Zurich Instruments MFLI: cannot set double {cmd}: device "
                + f"not connected."
            )
        ...
        if not self.LockIn is None:
            if ":" in cmd and len(cmd.split(":")) == 2:
                ...
                try:
                    ...
                    print(
                        f"Zurich Instruments MFLI could not set int "
                        + f"({cmd}): exception occured {e}"
                    )
            else:
                print(
            ...
            print(
                f"Zurich Instruments MFLI: cannot set int ({cmd}): device "
                + f"not connected."
            )
    ...
    def select_input_function(self, inp):
        self.set_int_function("demods/0/adcselect:" + SIGNAL_INPUTS[inp])
    ...
    def select_reference_function(self, ref):
        if ref == "internal":
            ...
            self.set_int_function("demods/1/adcselect:" + EXTERNAL_REFERENCES[ref])
        else:
            print(
                f"Zurich Instruments MFLI: oscillation reference can "
    ...
    def get_x_function(self):
        if not self.LockIn is None:
            try:
                ...
                print(
                    f"Zurich Instruments MFLI: cannot get x: exception "
                    + f"occured: {e}"
                )
            return 0
        else:
            print(f"Zurich Instruments MFLI: cannot get x: device " + f"not connected.")
            return 0
    ...
    def get_y_function(self):
        if not self.LockIn is None:
            try:
                ...
                print(
                    f"Zurich Instruments MFLI: cannot get y: exception "
                    + f"occured: {e}"
                )
            return 0
        else:
            print(f"Zurich Instruments MFLI: cannot get y: device " + f"not connected.")
            return 0
    ...
    def get_R_function(self):
        if not self.LockIn is None:
            try:
                s = self.LockIn.getSample(f"/{self.dev_name}/demods/0/sample")
                return (s["x"][0] ** 2 + s["y"][0] ** 2) ** 0.5
            except Exception as e:
                print(
                    f"Zurich Instruments MFLI: cannot get R: exception "
                    + f"occured: {e}"
                )
            return 0
        else:
            print(f"Zurich Instruments MFLI: cannot get R: device " + "not connected.")
            return 0
    ...
    def get_theta_function(self):
        if not self.LockIn is None:
            try:
                ...
                print(
                    f"Zurich Instruments MFLI: cannot get theta: exception "
                    + f"occured: {e}"
                )
            return 0
        else:
            print(
                f"Zurich Instruments MFLI: cannot get theta: device "
                + f"not connected."
            )
            return 0
