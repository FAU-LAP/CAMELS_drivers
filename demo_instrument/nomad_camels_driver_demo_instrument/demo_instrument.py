...
class subclass_config(device_class.Device_Config):
    def __init__(
        ...
        **kwargs
        ...
        detector_noises = (
            settings_dict["detector_noises"]
            if "detector_noises" in settings_dict
            else [0, 0, 0]
        )
        motor_noises = (
            settings_dict["motor_noises"]
            if "motor_noises" in settings_dict
            else [0, 0, 0]
        )
        set_delays = (
            settings_dict["set_delays"] if "set_delays" in settings_dict else [0, 0, 0]
        )
        system_delays = (
            settings_dict["system_delays"]
            if "system_delays" in settings_dict
            else [0, 0, 0]
        )
