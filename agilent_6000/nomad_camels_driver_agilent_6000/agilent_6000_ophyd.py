...
class Agilent_6000(VISA_Device):
    ...
    math_data = Cpt(VISA_Signal_RO, name='math_data')
    ...
    def make_screenshot(self):
        self.visa_instrument.write(f':HARD:INKS {self.invert_colors.get()};')
        write_string = ':DISP:DATA? '
        if self.image_type.get() == 'bmp':
            write_string += 'BMP, '
        elif self.image_type.get() == 'bmp8bit':
            write_string += 'BMP8bit, '
        elif self.image_type.get() == 'png':
            write_string += 'PNG, '
        if self.image_type.get() == 'tiff':
    ...
    def fetch_analog(self, source):
        ...
        y_data = self.visa_instrument.query(':WAV:DATA?')
    ...
    def fetch_digital(self, pod_source):
        ...
        n_points = int(values.group(3))
        x_increment = float(values.group(5))
        y_data = list(y_data)
        y_data = [int_to_bools(n) for n in y_data]
        y_data = np.array(pad_bool_arrays(y_data))
        dt = n_points / len(y_data) * x_increment
        x_data = np.array([dt * i for i in range(len(y_data))])
        for i in range(0, 2):
