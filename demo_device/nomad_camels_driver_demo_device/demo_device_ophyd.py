...
class Demo_Device(Device):
    ...
    def __init__(self, prefix='', *, name, kind=None, read_attrs=None,
        ...
        self.motorX.write = lambda x: self.motor_func(x, 0)
        self.motorY.write = lambda x: self.motor_func(x, 1)
        self.motorZ.write = lambda x: self.motor_func(x, 2)
        self.motorX.query = lambda: self.motor_read_func(0)
        self.motorY.query = lambda: self.motor_read_func(1)
        self.motorZ.query = lambda: self.motor_read_func(2)
        self.detectorX.query = lambda: self.det_func(0)
        self.detectorY.query = lambda: self.det_func(1)
        self.detectorZ.query = lambda: self.det_func(2)
        self.detectorComm.query = lambda: self.det_func(3)
