...
is_allowed_voltage_integration_time = (
    lambda voltage_integration_time: is_in_allowed_margins(
        voltage_integration_time, ALLOWED_VOLTAGE_INTEGRATION_TIME_MARGINS
    )
)
is_allowed_npl_cycles = lambda npl_cycles: is_in_allowed_margins(
    npl_cycles, ALLOWED_NPL_CYCLES_MARGINS
)
# ------------------------------------------------------------------------------
...
class Keithley_2182A(VISA_Device):
    ...
    select_voltage_measurement = Cpt(VISA_Signal, name="select_voltage_measurement")
    select_channel = Cpt(VISA_Signal, name="select_channel")
    measurement = Cpt(VISA_Signal_RO, name="measurement", query=":FETC?")
    set_voltage_integration_time = Cpt(VISA_Signal, name="set_voltage_integration_time")
    set_npl_cycles = Cpt(VISA_Signal, name="set_npl_cycles")
    set_to_calculate_resistance = Cpt(
        VISA_Signal, name="set_to_calculate_resistance", metadata={"units": "A"}
    )
    ...
    def set_voltage_integration_time_function(self, integration_time):
        if is_allowed_voltage_integration_time(integration_time):
            ...
            print(
                "Keithley 2182A nanovoltmeter: voltage integration time "
    ...
    def autorange_channel_function(self, channel):
        if channel in ALLOWED_CHANNELS:
            ...
            print(
                "Keithley 2182A nanovoltmeter: autorange channel "
    ...
    def select_channel_function(self, channel):
        ...
        if channel in ALLOWED_CHANNELS:
            ...
            print(
                "Keithley 2182A nanovoltmeter: select channel "
                + f"{channel} not allowed; allowed channels: "
                + f"{ALLOWED_CHANNELS}"
            )
        return ""
    ...
    def set_npl_cycles_function(self, npl_cycles):
        if is_allowed_npl_cycles(npl_cycles):
            ...
            print(
                "Keithley 2182A nanovoltmeter: NPL cycles"
