        ...
        **kwargs,
        ...
        for key, val in config_dict.items():
            if key.endswith("1"):
                ...
                conf2[key[:-1]] = val
        self.channel_widge_1 = subclass_config_sub(
            config_dict=conf1, parent=parent, settings_dict=settings_dict
        )
        self.channel_widge_2 = subclass_config_sub(
            config_dict=conf2, parent=parent, settings_dict=settings_dict
        )
...
class subclass_config_sub(device_class.Device_Config_Sub, Ui_B2912_channel):
    def __init__(self, config_dict=None, parent=None, settings_dict=None):
        ...
        if (
            "low_terminal" in config_dict
            and config_dict["low_terminal"] in self.low_terminals
        ):
        ...
        if "current_compliance" in config_dict:
            self.lineEdit_current_compliance.setText(
                str(config_dict["current_compliance"])
            )
        else:
            self.lineEdit_current_compliance.setText("0.1")
        if "voltage_compliance" in config_dict:
            self.lineEdit_voltage_compliance.setText(
                str(config_dict["voltage_compliance"])
            )
        else:
        ...
        if "current_auto_range" in config_dict:
            self.checkBox_current_auto_range.setChecked(
                config_dict["current_auto_range"]
            )
        if "voltage_auto_range" in config_dict:
            self.checkBox_voltage_auto_range.setChecked(
                config_dict["voltage_auto_range"]
            )
        if "resistance_auto_range" in config_dict:
            self.checkBox_resistance_auto_range.setChecked(
                config_dict["resistance_auto_range"]
            )
        if "resistance_compensation" in config_dict:
            self.checkBox_resistance_compensation.setChecked(
                config_dict["resistance_compensation"]
            )
        ...
        if (
            "voltage_auto_mode" in config_dict
            and config_dict["voltage_auto_mode"] in auto_range_modes
        ):
            self.comboBox_voltage_auto_mode.setCurrentText(
                config_dict["voltage_auto_mode"]
            )
        if (
            "current_auto_mode" in config_dict
            and config_dict["current_auto_mode"] in auto_range_modes
        ):
            self.comboBox_current_auto_mode.setCurrentText(
                config_dict["current_auto_mode"]
            )

        self.comboBox_current_lower_lim.addItems(self.ranges_current)
        if (
            "current_lower_lim" in self.config_dict
            and self.config_dict["current_lower_lim"] in self.ranges_current
        ):
            self.comboBox_current_lower_lim.setCurrentText(
                self.config_dict["current_lower_lim"]
            )
        self.comboBox_current_range.addItems(self.ranges_current)
        if (
            "current_range" in self.config_dict
            and self.config_dict["current_range"] in self.ranges_current
        ):
            self.comboBox_current_range.setCurrentText(
                self.config_dict["current_range"]
            )
        self.comboBox_voltage_lower_lim.addItems(self.ranges_voltage)
        if (
            "voltage_lower_lim" in self.config_dict
            and self.config_dict["voltage_lower_lim"] in self.ranges_voltage
        ):
            self.comboBox_voltage_lower_lim.setCurrentText(
                self.config_dict["voltage_lower_lim"]
            )
        self.comboBox_voltage_range.addItems(self.ranges_voltage)
        if (
            "voltage_range" in self.config_dict
            and self.config_dict["voltage_range"] in self.ranges_voltage
        ):
            self.comboBox_voltage_range.setCurrentText(
                self.config_dict["voltage_range"]
            )
        self.comboBox_resistance_range.addItems(self.ranges_resistance)
        if (
            "resistance_range" in self.config_dict
            and self.config_dict["resistance_range"] in self.ranges_resistance
        ):
            self.comboBox_resistance_range.setCurrentText(
                self.config_dict["resistance_range"]
            )
        self.comboBox_resistance_upper_lim.addItems(self.ranges_resistance)
        if (
            "resistance_upper_lim" in self.config_dict
            and self.config_dict["resistance_upper_lim"] in self.ranges_resistance
        ):
            self.comboBox_resistance_upper_lim.setCurrentText(
                self.config_dict["resistance_upper_lim"]
            )
    ...
    def load_source_options(self):
        ...
        if src_v:
            ...
            if (
                "source_range" in self.config_dict
                and self.config_dict["source_range"] in self.ranges_voltage
            ):
                self.comboBox_source_range.setCurrentText(
                    self.config_dict["source_range"]
                )
            if (
                "range_lower_lim" in self.config_dict
                and self.config_dict["range_lower_lim"] in self.ranges_voltage
            ):
                self.comboBox_range_lower_lim.setCurrentText(
                    self.config_dict["range_lower_lim"]
                )
        else:
            ...
            if (
                "source_range" in self.config_dict
                and self.config_dict["source_range"] in self.ranges_current
            ):
                self.comboBox_source_range.setCurrentText(
                    self.config_dict["source_range"]
                )
            if (
                "range_lower_lim" in self.config_dict
                and self.config_dict["range_lower_lim"] in self.ranges_current
            ):
                self.comboBox_range_lower_lim.setCurrentText(
                    self.config_dict["range_lower_lim"]
                )
    ...
    def get_config(self):
        ...
        self.config_dict["range_lower_lim"] = (
            self.comboBox_range_lower_lim.currentText()
        )
        self.config_dict["current_auto_mode"] = (
            self.comboBox_current_auto_mode.currentText()
        )
        self.config_dict["current_lower_lim"] = (
            self.comboBox_current_lower_lim.currentText()
        )
        ...
        self.config_dict["voltage_auto_mode"] = (
            self.comboBox_voltage_auto_mode.currentText()
        )
        self.config_dict["voltage_lower_lim"] = (
            self.comboBox_voltage_lower_lim.currentText()
        )
        self.config_dict["resistance_range"] = (
            self.comboBox_resistance_range.currentText()
        )
        self.config_dict["resistance_upper_lim"] = (
            self.comboBox_resistance_upper_lim.currentText()
        )
        self.config_dict["source_auto"] = self.checkBox_source_auto.isChecked()
        self.config_dict["output_protection"] = (
            self.checkBox_output_protection.isChecked()
        )
        self.config_dict["four_wire_meas"] = self.checkBox_four_wire_meas.isChecked()
        self.config_dict["current_auto_range"] = (
            self.checkBox_current_auto_range.isChecked()
        )
        self.config_dict["voltage_auto_range"] = (
            self.checkBox_voltage_auto_range.isChecked()
        )
        self.config_dict["resistance_auto_range"] = (
            self.checkBox_resistance_auto_range.isChecked()
        )
        self.config_dict["resistance_compensation"] = (
            self.checkBox_resistance_compensation.isChecked()
        )
        self.config_dict["current_compliance"] = float(
            self.lineEdit_current_compliance.text()
        )
        self.config_dict["voltage_compliance"] = float(
            self.lineEdit_voltage_compliance.text()
        )
