...
ALLOWED_WAVE_TYPE = ["SIN", "SQU", "RAMP", "ARB0", "ARB1", "ARB2", "ARB3", "ARB4"]
...
is_allowed_current = lambda current: is_in_allowed_margins(
    current, ALLOWED_CURRENT_MARGINS
)
is_allowed_wave_amplitude = lambda amplitude: is_in_allowed_margins(
    amplitude, ALLOWED_WAVE_AMPLITUDE_MARGINS
)
is_allowed_wave_frequency = lambda frequency: is_in_allowed_margins(
    frequency, ALLOWED_WAVE_FREQUENCY_MARGINS
)
is_allowed_wave_duty_cycle = lambda duty_cycle: is_in_allowed_margins(
    duty_cycle, ALLOWED_WAVE_DUTY_CYCLE_MARGINS
)
is_allowed_wave_offset = lambda offset: is_in_allowed_margins(
    offset, ALLOWED_WAVE_OFFSET
)
# ------------------------------------------------------------------------------
...
class Keithley_6221(VISA_Device):
    ...
    set_constant_current = Cpt(
        VISA_Signal, name="set_constant_current", metadata={"units": "A"}
    )
    enable_output = Cpt(VISA_Signal, name="enable_output")
    reset = Cpt(VISA_Signal, write="*RST", name="reset")
    phase_marker_trigger_line = Cpt(
        VISA_Signal, name="phase_marker_trigger_line", kind="config"
    )
    enable_phase_marker = Cpt(VISA_Signal, name="enable_phase_marker", kind="config")
    set_phase_marker_phase = Cpt(
        VISA_Signal, name="set_phase_marker_phase", kind="config"
    )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.phase_marker_trigger_line.write = self.phase_marker_trigger_line_function
    ...
    def set_constant_current_function(self, current):
        if is_allowed_current(current):
            ...
            print(
                "Keithley 6221 current source: current {current} A not in "
                + f"allowed margins ({ALLOWED_CURRENT_MARGINS[0]} A to "
                + f"{ALLOWED_CURRENT_MARGINS[1]}) A",
                flush=1,
            )
        return ""
    ...
    def phase_marker_trigger_line_function(self, line):
        ...
        if line in ALLOWED_PMAR_LINES:
            ...
            print(
                "Keithley 6221 current source: available lines for phase "
    ...
    def enable_phase_marker_function(self, state):
        ...
        self.visa_instrument.write(
            "SOUR:WAVE:PMAR:STAT " + f'{"ON" if state else "OFF"}'
        )
        return ""
    ...
    def set_wave_type_function(self, wave_type):
        ...
        if wave_type in ALLOWED_WAVE_TYPE:
            ...
            print(
                f'Keithley 6221 current source: wave type "{wave_type}"'
    ...
    def set_wave_amplitude_function(self, amplitude):
        if is_allowed_wave_amplitude(amplitude):
            ...
            print(
                f"Keithley 6221 current source: amplitude {amplitude} Ap2p"
    ...
    def set_wave_frequency_function(self, frequency):
        if is_allowed_wave_frequency(frequency):
            ...
            print(
                f"Keithley 6221 current source: frequency {frequency} Hz"
    ...
    def set_wave_duty_cycle_function(self, duty_cycle):
        if is_allowed_wave_duty_cycle(duty_cycle):
            ...
            print(
                f'Keithley 6221 current source: duty cycle "{duty_cycle}"'
    ...
    def set_wave_offset_function(self, offset):
        if is_allowed_wave_offset(offset):
            ...
            print(
                f"Keithley 6221 current source: wave offset {offset} A"
