...
class TMCM_1110(Device):
    ...
    def __init__(
        ...
        *,
        ...
        parent=None,
        connection_port="",
        search_reference_on_start=True,
        motor_number=0,
        **kwargs,
    ):
        super().__init__(
        ...
        try:
            self.interface = ConnectionManager(
                f"--port COM{connection_port_match.group(1)}"
            ).connect()
        except:
        ...
        ap = self.motor.AP
        self.ap_names = {
            "ref_search_mode": ap.ReferenceSearchMode,
        ...
        self.ref_search_mode.put_function = lambda x: self.set_parameter(
            x, "ref_search_mode"
        )
        self.right_lim_switch_disable.put_function = lambda x: self.set_parameter(
            x, "right_lim_switch_disable"
        )
        self.left_lim_switch_disable.put_function = lambda x: self.set_parameter(
            x, "left_lim_switch_disable"
        )
        self.ref_search_speed.put_function = lambda x: self.set_parameter(
            x, "ref_search_speed"
        )
        self.ref_switch_speed.put_function = lambda x: self.set_parameter(
            x, "ref_switch_speed"
        )
        self.max_acceleration.put_function = lambda x: self.set_parameter(
            x, "max_acceleration"
        )
        self.max_velocity.put_function = lambda x: self.set_parameter(x, "max_velocity")
        self.power_down_delay.put_function = lambda x: self.set_parameter(
            x, "power_down_delay"
        )
        self.standby_current.put_function = lambda x: self.set_parameter(
            x, "standby_current"
        )
        self.max_current.put_function = lambda x: self.set_parameter(x, "max_current")
        self.freewheeling_delay.put_function = lambda x: self.set_parameter(
            x, "freewheeling_delay"
        )
        self.pulse_divisor.put_function = lambda x: self.set_parameter(
            x, "pulse_divisor"
        )
        self.ramp_divisor.put_function = lambda x: self.set_parameter(x, "ramp_divisor")
        self.soft_stop_flag.put_function = lambda x: self.set_parameter(
            x, "soft_stop_flag"
        )
        self.microstep_resolution.put_function = lambda x: self.set_parameter(
            x, "microstep_resolution"
        )
    ...
    def find_reference(self):
        ...
        while self.interface.reference_search(
            2, self.motor_number, self.module.module_id
        ):
            time.sleep(0.1)
