...
class Lakeshore_F41(Device):
    set_field = Cpt(Custom_Function_Signal, name="set_field", metadata={"units": "T"})
    get_field = Cpt(Custom_Function_SignalRO, name="get_field", metadata={"units": "T"})
    set_P = Cpt(Custom_Function_Signal, name="set_P", kind="config")
    set_I = Cpt(Custom_Function_Signal, name="set_I", kind="config")
    set_D = Cpt(
        Custom_Function_Signal, name="set_D", kind="config", metadata={"units": "mT/s"}
    )
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not self.name == "test":
            ...
            self.tm.configure_field_control_output_mode(
                mode="CLLOOP", output_enabled=True
            )
